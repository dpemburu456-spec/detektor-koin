# app.py (tanpa streamlit_autorefresh)
...
# Tombol refresh manual dan auto-refresh logika
auto_refresh = st.sidebar.checkbox("✅ Auto-refresh otomatis", value=True)
if auto_refresh:
    elapsed = time.time() - st.session_state.last_update
    if elapsed >= refresh_interval:
        st.session_state.last_update = time.time()
        st.rerun()
    st.sidebar.info(f"Refresh otomatis dalam {int(refresh_interval - elapsed)} detik")
else:
    if st.sidebar.button("🔄 Refresh Sekarang"):
        st.rerun()

st.sidebar.markdown("---")
st.sidebar.caption("Data diambil dari API publik Indodax")

# Header
st.title("📈 Indodax Pump Detector")
st.markdown(f"Memantau **{threshold}%** kenaikan dalam **{period} detik** | Refresh setiap **{refresh_interval} detik** (otomatis jika dicentang)")
...
with st.expander("📋 Daftar Semua Pasangan yang Dipantau (50 teratas)"):
...
with st.expander("ℹ️ Status Detektor"):
    ...
    st.write("Snapshot diperbarui setiap refresh. Deteksi terjadi setelah periode yang ditentukan.")